...
async function run() {
  try {
    ...
    const availablePackingLotsWithoutCompletedFilter = [];
    ...
    for (let i = 1; i < indexRows.length; i++) {
      ...
      if (!inIssues && !inRawpackAssigned) {
        availablePackingLotsWithoutCompletedFilter.push({ lotNo, supervisorVal, partyVal });
      }
    }

    console.log(`\nAvailable Packing Lots WITHOUT Stitching Completed Filter: ${availablePackingLotsWithoutCompletedFilter.length}`);
    console.log('Sample 10:', availablePackingLotsWithoutCompletedFilter.slice(0, 10));
