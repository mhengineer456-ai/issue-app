...
async function run() {
  try {
    ...
    const excludedSupervisorsCount = [];
    ...
    for (let i = 1; i < indexRows.length; i++) {
      ...
      if (lotNo && completedStatus !== '' && completedStatus !== '-') {
        ...
        if (isDushyantOrJain) {
          excludedSupervisorsCount.push({ lotNo, supervisorVal, partyVal });
          continue;
        }
        ...
        if (!inIssues && !inRawpackAssigned) {
          availablePackingLots.push({ lotNo, supervisorVal, partyVal });
        }
      }
    }

    console.log(`Excluded Lots with Supervisor/Party Dushyant/Jainhosiery: ${excludedSupervisorsCount.length}`);
    console.log('Sample Excluded:', excludedSupervisorsCount.slice(0, 5));
    console.log(`\nRemaining Available Packing Lots: ${availablePackingLots.length}`);
    console.log('Sample Remaining:', availablePackingLots.slice(0, 10));
